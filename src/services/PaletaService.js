...
const paletasComId = paletas.map((paleta, index) => ({
  ...paleta,
  id: index,
...
export const PaletaService = {
  getLista: () =>
    new Promise((resolve) => {
      setTimeout(() => {
        resolve(paletasComId);
      }, 1000);
    }),
  getById: (id) => paletasComId.find((paleta) => paleta.id === id),
  updateById: (id, paleta) => {
    const index = paletasComId.findIndex((p) => p.id === id);
    if (index !== -1) {
      paletasComId[index] = { ...paletasComId[index], ...paleta };
    }
  },
  deleteById: (id) => {
    const index = paletasComId.findIndex((p) => p.id === id);
    if (index !== -1) {
      paletasComId.splice(index, 1);
    }
  },
  create: (paleta) => {
    const newPaleta = { ...paleta, id: paletasComId.length };
    paletasComId.push(newPaleta);
    return newPaleta;
  },
};
